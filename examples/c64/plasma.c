// Copyright 2023 LLVM-MOS Project
// Licensed under the Apache License, Version 2.0 with LLVM Exceptions.
// See https://github.com/llvm-mos/llvm-mos-sdk/blob/main/LICENSE for license
// information.

/*
 * Simplistic character-mode plasma effect
 *
 * Inspired by cc65/samples/cbm
 * - 2001 by groepaz
 * - Cleanup and C porting by Ullrich von Bassewitz
 * - 2023 LLVM-MOS adaptation by Mikael Lund aka Wombat
 */

#include <c64.h>
#include <peekpoke.h>
#include <stdint.h>
#include <stdio.h>

enum {
  SCREEN = 0x0400,
  CHARSET = 0x2000,
  SID_NOISE_BIT = 0x80,
  SINE_REPEAT = 8,
  COLS = 40,
  ROWS = 25
};

/* Cyclic sine lookup table */
static const uint8_t sine_table[0x100] = {
    };

/*
 * Tell SID to start generating noise waveform as source for randomness
 * https://www.atarimagazines.com/compute/issue72/random_numbers.php
 */
void start_sid_random_generator(void) {
  struct __sid_voice voice;
  voice.freq = UINT16_MAX;
  voice.ctrl = SID_NOISE_BIT;
  SID.v3 = voice;
}

/* Pseudo random number generator using SID noise */
uint8_t rand8(void) { return SID.noise; }

/* Generate charset with 8 * 256 characters */
void make_charset(void) {
  static const uint8_t bits[8] = {0x01, 0x02, 0x04, 0x08,
                                  0x10, 0x20, 0x40, 0x80};

  for (size_t c = 0; c < sizeof(sine_table); ++c) {
    const uint8_t sine = sine_table[c];
    for (size_t i = 0; i < SINE_REPEAT; ++i) {
      uint8_t pattern = 0;
      for (size_t j = 0; j < sizeof(bits); ++j) {
        if (rand8() > sine) {
          pattern |= bits[j];
        }
      }
      POKE(CHARSET + (c * 8) + i, pattern);
    }
    VIC.bordercolor++;
  }
}

/* Plasma effect */
static void do_plasma(volatile uint8_t *screen_address) {
  static uint8_t xdata[COLS];
  static uint8_t ydata[ROWS];
  static uint8_t x_cnt1 = 0;
  static uint8_t x_cnt2 = 0;
  static uint8_t y_cnt1 = 0;
  static uint8_t y_cnt2 = 0;

  uint8_t k = y_cnt1;
  uint8_t l = y_cnt2;
  for (size_t i = 0; i < sizeof(ydata); ++i, k += 4, l += 9) {
    ydata[i] = sine_table[k] + sine_table[l];
  }
  y_cnt1 += 3;
  y_cnt2 -= 5;

  k = x_cnt1;
  l = x_cnt2;
  for (size_t i = 0; i < sizeof(xdata); ++i, k += 3, l += 7) {
    xdata[i] = sine_table[k] + sine_table[l];
  }
  x_cnt1 += 2;
  x_cnt2 -= 3;

  // Write summed buffers to screen memory
  for (size_t i = 0; i < sizeof(ydata); ++i) {
#pragma unroll
    for (size_t j = 0; j < sizeof(xdata); ++j, ++screen_address) {
      POKE(screen_address, xdata[j] + ydata[i]);
    }
  }
}

int main(void) {
  start_sid_random_generator();
  make_charset();
  VIC.addr = ((SCREEN >> 6U) & 0xF0U) | ((CHARSET >> 10U) & 0x0EU);
  while (1) {
    do_plasma((uint8_t *)SCREEN);
  }
}
