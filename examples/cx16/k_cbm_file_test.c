...
const char payload[] = "THIS IS A TEXT FILE CREATED BY LLVM-MOS\r"
                       "ON THE COMMANDER X16 USING CBM FILE I/O.\r";

const char payload2[] = "THIS IS A COMPLETELY DIFFERNT STRING TO \r"
...
int main(void) {
  ...
  if (!cbm_error) {
    printf("Write: ");
    len = (int16_t)cbm_file_write(LFN_FD1, payload,
                                  sizeof(payload) - 1); // don't write NUL char
    if (len >= 0) {
      printf("%d bytes, ", len);
    }
    show_status();
  ...
  if (!cbm_error) {
    printf("Read: ");
    len = (int16_t)cbm_file_read(LFN_FD1, read_buffer,
                                 sizeof(read_buffer) - 1); // leave  NUL char
    if (len >= 0) {
      printf("%d bytes, ", len);
    }
    show_status();
  ...
  cbm_file_write(LFN_FD2, payload2,
                 sizeof(payload2) - 1); // don't write NUL char
  show_status();
  printf(" Write \"%s\": ", test1name);
  cbm_file_write(LFN_FD1, payload, sizeof(payload) - 1); // don't write NUL char
  show_status();
