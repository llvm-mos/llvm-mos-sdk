...
noinline void FileIn()
{
    SwapMod(MOD_FILE);
}

noinline void EditIn()
{
    SwapMod(MOD_EDIT);
}

noinline void R_DoAbout(void)
{
    GotoFirstMenu();
}
...
menu_tab_t GeosSubMenu = {
    .top = SM_TOP, .bot = SM_TOP + 1 + 1 * 14,
    .left = GM_LEFT, .right = GM_LEFT + GM_WIDTH,
    .menu_type_and_count = VERTICAL | GM_COUNT,
    .items = {
        { .text = "Sample VLIR info", .action_type = MENU_ACTION, .handler_function = R_DoAbout },
        { .text = DAText[0], .action_type = MENU_ACTION, .handler_function = R_RunDA0 },
        { .text = DAText[1], .action_type = MENU_ACTION, .handler_function = R_RunDA1 },
        { .text = DAText[2], .action_type = MENU_ACTION, .handler_function = R_RunDA2 },
        { .text = DAText[3], .action_type = MENU_ACTION, .handler_function = R_RunDA3 },
        { .text = DAText[4], .action_type = MENU_ACTION, .handler_function = R_RunDA4 },
        { .text = DAText[5], .action_type = MENU_ACTION, .handler_function = R_RunDA5 },
        { .text = DAText[6], .action_type = MENU_ACTION, .handler_function = R_RunDA6 },
        { .text = DAText[7], .action_type = MENU_ACTION, .handler_function = R_RunDA7 },
    }
};

menu_tab_t FileSubMenu = {
    .top = SM_TOP, .bot = SM_TOP + 1 + FM_COUNT *14,
    .left = FM_LEFT, .right = FM_LEFT + FM_WIDTH,
    .menu_type_and_count = VERTICAL | FM_COUNT,
    .items = {
        { .text = "close", .action_type  = MENU_ACTION, .handler_function = R_DoClose},
        { .text = "quit", .action_type = MENU_ACTION, .handler_function = R_DoQuit}
    }
};
