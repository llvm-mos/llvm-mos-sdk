...
void print_bcd_int(BcdVarInt &value, bool negative, Status *status) {
  ...
  if (status->flags & E_alt) {
    if (status->base == 8) {
      if (prec < value.size() + 1)
        prec = value.size() + 1;
    }
  }
  ...
  if (status->flags & E_alt && status->base == 16) {
    prefix[0] = '0';
  ...
  size_t padding = (status->width > size) ? status->width - size : 0;
  ...
  if (!(status->flags & E_minus) && padding) {
    while (padding--)
      put(' ', status);
  }
