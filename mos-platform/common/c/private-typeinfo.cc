...
static inline bool is_equal(const std::type_info *x, const std::type_info *y,
                            bool use_strcmp) {
...
void __class_type_info::process_found_base_class(
    __dynamic_cast_info *info, void *adjustedPtr,
...
//                     -3: static_type is a multiple public base type but never
//                     a
//                         virtual base type
...
// node of the tree represents a base class/object of its parent (or parents)
// below. Each node is uniquely represented by a pointer to the object, and a
// pointer to a type_info - its type.  Different nodes may have the same pointer
// and different nodes may have the same type.  But only one node has a specific
...
//
// dst_type != static_type:  The compiler computes the dynamic_cast in this case
// too. dynamic_type != static_type:  The compiler computes the dynamic_cast in
// this case too.
//
// Returns:
//
// If there is exactly one dst_type of flavor 1, and
//    If there is a public path from that dst_type to (static_ptr, static_type),
//    or If there are 0 dst_types of flavor 2, and there is a public path from
...
// Else if there are 0 dst_types of flavor 1 and exactly 1 dst_type of flavor 2,
// and
//    if there is a public path from (dynamic_ptr, dynamic_type) to
//    (static_ptr, static_type) and a public path from (dynamic_ptr,
//    dynamic_type) to the one dst_type, then return a pointer to that one
//    dst_type.
// Else return nullptr.
//
...
__dynamic_cast(const void *static_ptr, const __class_type_info *static_type,
  ...
  if (is_equal(dynamic_type, dst_type, false)) {
    ...
    // Do the  search
    dynamic_type->devirt_search_above_dst(&info, dynamic_ptr, dynamic_ptr,
                                          public_path, false);
    // Query the search.
    ...
    dynamic_type->devirt_search_below_dst(&info, dynamic_ptr, public_path,
                                          false);
...
// Call this function when you hit a static_type which is a base (above) a
// dst_type. Let caller know you hit a static_type.  But only start recording
// details if this is (static_ptr, static_type) -- the node we are casting from.
// If this is (static_ptr, static_type)
//   Record the path (public or not) from the dst_type to here.  There may be
//   multiple paths from the same dst_type to here, record the "most public"
//   one. Record the dst_ptr as pointing to (static_ptr, static_type). If more
//   than one (dst_ptr, dst_type) points to (static_ptr, static_type), then mark
//   this dyanmic_cast as ambiguous and stop the search.
void __class_type_info::process_static_type_above_dst(
    __dynamic_cast_info *info, const void *dst_ptr, const void *current_ptr,
    path_accessibility path_below) const {
...
// Call this function when you hit a static_type which is not a base (above) a
// dst_type. If this is (static_ptr, static_type)
...
// Call this function when searching below a dst_type node.  This function
// searches for a path to (static_ptr, static_type) and for paths to one or more
// dst_type nodes. If it finds a static_type node, there is no need to further
// search base classes above. If it finds a dst_type node it should search base
// classes using search_above_dst to find out if this dst_type points to
// (static_ptr, static_type) or not. Either way, the dst_type is recorded as one
// of two "flavors":  one that does or does not point to (static_ptr,
// static_type). If this is neither a static_type nor a dst_type node, continue
// searching base classes above. All the hoopla surrounding the search code is
// doing nothing but looking for excuses to stop the search prematurely (break
// out of the for-loop).  That is, the algorithm below is simply an optimization
// of this: void
...
//             // break out early here if you can detect it doesn't matter if
//             you do
//         }
...
//         for (Iter p = __base_info, e = __base_info + __base_count; p < e;
//         ++p)
//         {
...
//     }
// }
void __vmi_class_type_info::search_below_dst(__dynamic_cast_info *info,
                                             const void *current_ptr,
  ...
  if (is_equal(this, info->static_type, use_strcmp))
    ...
    } else // We have haven't been here before
    {
      ...
      if (info->is_dst_type_derived_from_static_type != no) {
        ...
        //    2.  We've found an ambiguous cast from (static_ptr, static_type)
        //    to a dst_type.
        //        This is detected at the (static_ptr, static_type).
        //    3.  We can prove that there is no public path to (static_ptr,
        //    static_type)
        //        above here.
    ...
    if (++p < e) {
      if ((__flags & __diamond_shaped_mask) ||
        ...
        do {
          if (info->search_done)
            break;
          // If we just found a dst_type with a public path to (static_ptr,
          // static_type),
        ...
        do {
          ...
          //    no other dst_type points to (static_ptr, static_type).
          //    If !diamond, then we don't need to search here.
          // if we just found a dst_type with a private path to (static_ptr,
          // static_type),
          //    then we're only looking for a public path to (static_ptr,
          //    static_type) and to check for other dst_types. If !diamond &
          //    !repeat, then there is not a pointer to (static_ptr,
          //    static_type) and not a dst_type under here.
          if (info->number_to_static_ptr == 1)
            break;
          p->search_below_dst(info, current_ptr, path_below, use_strcmp);
        } while (++p < e);
...
void __si_class_type_info::search_below_dst(__dynamic_cast_info *info,
                                            const void *current_ptr,
                                            path_accessibility path_below,
                                            bool use_strcmp) const {
  if (is_equal(this, info->static_type, use_strcmp))
    ...
    } else // We have haven't been here before
    {
      ...
      if (info->is_dst_type_derived_from_static_type != no) {
        ...
        info->found_our_static_ptr = false;
        info->found_any_static_type = false;
        __base_type->devirt_search_above_dst(info, current_ptr, current_ptr,
                                             public_path, use_strcmp);
        if (info->found_any_static_type) {
          is_dst_type_derived_from_static_type = true;
          if (info->found_our_static_ptr)
            does_dst_type_point_to_our_static_type = true;
    ...
    // This is not a static_type and not a dst_type
    __base_type->devirt_search_below_dst(info, current_ptr, path_below,
                                         use_strcmp);
  }
}

template <class type_info_most_derived>
void __class_type_info::invoke_search_below_dst(const __class_type_info *self,
                                                __dynamic_cast_info *info,
                                                const void *current_ptr,
                                                path_accessibility path_below,
                                                bool use_strcmp) {
...
void __class_type_info::search_below_dst(__dynamic_cast_info *info,
                                         const void *current_ptr,
                                         path_accessibility path_below,
                                         bool use_strcmp) const {
  if (is_equal(this, info->static_type, use_strcmp))
...
// Call this function when searching above a dst_type node.  This function
// searches for a public path to (static_ptr, static_type). This function is
// guaranteed not to find a node of type dst_type. Theoretically this is a very
// simple function which just stops if it finds a static_type node:  All the
// hoopla surrounding the search code is doing nothing but looking for excuses
// to stop the search prematurely (break out of the for-loop).  That is, the
// algorithm below is simply an optimization of this: void
...
// {
//     if (this == info->static_type)
//         process_static_type_above_dst(info, dst_ptr, current_ptr,
//         path_below);
//     else
//     {
//         typedef const __base_class_type_info* Iter;
...
//     }
// }
void __vmi_class_type_info::search_above_dst(__dynamic_cast_info *info,
                                             const void *dst_ptr,
                                             const void *current_ptr,
                                             path_accessibility path_below,
                                             bool use_strcmp) const {
  if (is_equal(this, info->static_type, use_strcmp))
    ...
    //  We can stop looking above if:
    //    1.  We've found a public path to (static_ptr, static_type).
    //    2.  We've found an ambiguous cast from (static_ptr, static_type) to a
    //    dst_type.
    //        This is detected at the (static_ptr, static_type).
    //    3.  We can prove that there is no public path to (static_ptr,
    //    static_type)
...
//   simplified to the case that there is only a single base class.
void __si_class_type_info::search_above_dst(__dynamic_cast_info *info,
                                            const void *dst_ptr,
                                            const void *current_ptr,
                                            path_accessibility path_below,
                                            bool use_strcmp) const {
  if (is_equal(this, info->static_type, use_strcmp))
    ...
    __base_type->devirt_search_above_dst(info, dst_ptr, current_ptr, path_below,
                                         use_strcmp);
}
...
// This is the same algorithm as __vmi_class_type_info::search_above_dst but
//   simplified to the case that there is no base class.
void __class_type_info::search_above_dst(__dynamic_cast_info *info,
                                         const void *dst_ptr,
                                         const void *current_ptr,
                                         path_accessibility path_below,
                                         bool use_strcmp) const {
...
// These two functions, search_above_dst and search_below_dst, are used to
// traverse the tree of type_info objects when executing the dynamic_cast
// function. In a "normal" cxxabi implementation, these would be virtual member
// functions on the types derived from __class_type_info. However, this has the
// side effect of making these functions defined in any program that generates
// RTTI instances and their associated vtables. This makes the resulting program
// larger even if the dynamic_cast functionality is not used: it's about 8K of
// text.

// Thus, a table of function pointers is used to provide a non-virtual call
// mechanism for these functions. Since these become normal function calls; the
// linker can easily find that they are not reachable from anywhere but the
// dynamic_cast function above, and therefore will completely optimize them out
// of the binary if they aren't reached. Note that the ABI definition requires
// the __class_type_info and derived classes to have a specific data member
// layout. We cannot modify any members of any of the classes, so instead we
// rely on RTTI type_info comparisons to locate the correct entries in the jump
// table for each of the __class_type_info descendants.
struct search_funcs {
...
void __base_class_type_info::search_above_dst(__dynamic_cast_info *info,
                                              const void *dst_ptr,
                                              const void *current_ptr,
                                              path_accessibility path_below,
                                              bool use_strcmp) const {
  ...
  __base_type->devirt_search_above_dst(
      info, dst_ptr, static_cast<const char *>(current_ptr) + offset_to_base,
      (__offset_flags & __public_mask) ? path_below : not_public_path,
      use_strcmp);
}

void __base_class_type_info::search_below_dst(__dynamic_cast_info *info,
                                              const void *current_ptr,
                                              path_accessibility path_below,
                                              bool use_strcmp) const {
  ptrdiff_t offset_to_base = __offset_flags >> __offset_shift;
  ...
  __base_type->devirt_search_below_dst(
      info, static_cast<const char *>(current_ptr) + offset_to_base,
      (__offset_flags & __public_mask) ? path_below : not_public_path,
      use_strcmp);
}
