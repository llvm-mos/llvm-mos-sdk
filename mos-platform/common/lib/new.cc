 ...
 *  "As described in Section 2.8, certain objects with static storage duration have 
 *   associated guard variables used to support the requirement that they be initialized 
 *   exactly once, the first time the scope declaring them is entered. An implementation 
 *   that does not anticipate supporting multi-threading may simply check the first byte 
 *   (i.e., the byte with lowest address) of that guard variable, initializing if and only 
 *   if its value is zero, and then setting it to a non-zero value."
 * 
 * Since this is intended to be a simple single-threaded implementation, we take the advise 
 * presented and just initialize the first byte of the 64-bit guard object.  The remaining 7
 * bytes are unused (but required to be present by the ABI standard)
 */
