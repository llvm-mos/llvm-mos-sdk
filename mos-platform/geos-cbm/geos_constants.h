...
typedef signed char int8_t;                 /* a signed byte */
...
#define TOTAL_BLOCKS            664 /* number of blocks on disk, not including 
                                     * directory track. */
...
#define OFF_OP_TR_SC            171     /* track and sector for off page directory
                                         * entries. 8 files may be moved off page. */
#define OFF_GS_ID               173     /* where GEOS ID string is located */
#define OFF_GS_DTYPE            189     /* GEOS disk type. Currently, is 0 for
...
#define ST_WR_PR                0x40    /* write protect bit: bit 6 of byte 0 in the
                                         * directory entry */
...
#define O_128_FLAGS             96      /* 1 byte, flag to indicate if this program
...
#define O_GHINFO_TXT            0xa0    /* offset to notes that are stored with the file
...
#define O_GHP_FNAME             117     /* 20 bytes: permanent filename of parent application */
...
#define ST_LD_AT_ADDR           0x01    /* "Load At Address": Load file a called
                                         * specified address instead of address file was
                                         * saved from. */
#define ST_LD_DATA              0x80    /* "Load Datafile": Used when application 
...
#define ST_PR_DATA              0x40    /* "Print Datafile": Used when application
...
#define DIR_1581_TRACK          40      /* track # reserved on disk for directory on a 1581 */
...
#define OK                      1       /* Put up system Icon for "OK", command is
...
#define DB_NAME(name) extern uint8_t name[]; asm(".type " STRINGIFY(name) ",@object\n.section .data." STRINGIFY(name) ",\"aw\",@progbits\n" STRINGIFY(name) ":");
#define DB_NAME_SECT(name, sect) extern uint8_t name[]; asm(".type " STRINGIFY(name) ",@object\n.section ." STRINGIFY(sect) "." STRINGIFY(name) ",\"aw\",@progbits\n" STRINGIFY(name) ":");
#define DB_END(name) asm(".byte 0\n.size " STRINGIFY(name) ", . - " STRINGIFY(name));
#define DB_DEFPOS(pattern) asm(".byte 0x80|" STRINGIFY(pattern));
#define DB_SETPOS(pattern, top, bottom, left, right) asm(".byte " STRINGIFY(pattern) "\n.byte " STRINGIFY(top) "\n.byte " STRINGIFY(bottom) "\n.word " STRINGIFY(left) "\n.word " STRINGIFY(right));
#define DB_ICON(icon, x, y) asm(".byte " STRINGIFY(icon) "\n.byte " STRINGIFY(x) "\n.byte " STRINGIFY(y));
#define DB_TXTSTR(x, y, text) asm(".byte 11\n.byte " STRINGIFY(x) "\n.byte " STRINGIFY(y) "\n.short " STRINGIFY(text));
#define DB_VARSTR(x, y, zp_ptr) asm(".byte 12\n.byte " STRINGIFY(x) "\n.byte " STRINGIFY(y) "\n.byte " STRINGIFY(zp_ptr));
#define DB_GETSTR(x, y, zp_ptr, length) asm(".byte 13\n.byte " STRINGIFY(x) "\n.byte " STRINGIFY(y) "\n.byte " STRINGIFY(zp_ptr) "\n.byte " STRINGIFY(length));
...
#define DB_USRICON(x, y, icon_ptr) asm(".byte 18\n.byte " STRINGIFY(x) "\n.byte " STRINGIFY(y) "\n.short " STRINGIFY(icon_ptr));
...
#define VIC_YPOS_OFF            50      /* Position offset from 0 to position a 
                                         * hardware sprite at the top of the screen.
                                         * Used to map from GEOS coordinates to hardware
                                         * position coordinates. */

#define VIC_XPOS_OFF            24      /* As above, offset from hardware 0 position to left
                                         * of screen, used to map GEOS coordinates in VIC. */

#define ALARMMASK               0b00000100  /* mask for the alarm bit in the CIA chip 
