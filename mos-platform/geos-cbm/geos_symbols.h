...
extern uint8_t baselineOffset;      /* Offset from top line to baseline in character set */
...
extern file_header_t fileHeader;    /* block used to hold file head block for a GEOS file. */
extern dir_header_t curDirHead;     /* block contains directory header information for 
                                     * disk in currently selected drive. */
extern file_tr_se_tab_t fileTrScTab;/* buffer used to hold track and sector chain for 
...
extern disk_name_t DrBCurDkNm;      /* Disk name of disk in drive B
                                     * 18 char disk name (padded with 0xa0) */
extern file_name_t dataFileName;    /* Name of data file (passed to application) */
...
extern vector intTopVector;         /* Called at the top of OS interrupt code to allow 
...
extern vector mouseFaultVec;        /* routine to call when mouse goes outside region or off a menu */
extern vector otherPressVec;        /* routine to call on mouse press that is not a menu or an icon */
extern vector StringFaultVec;       /* vector for when character is written over right margin */
extern vector alarmTmtVector;       /* address of a service routine for the alarm clock time-out
                                     * (ringing, graphic, etc.) that the App. can use if necessary.
                                     * Normally 0. */

extern vector BRKVector;            /* routine called when BRK encountered */
extern vector RecoverVector;        /* routine called to recover background behind menus and
                                     * dialog boxes */
extern uint8_t selectionFlash;      /* speed at which menu items and icons are flashed */
...
extern uint8_t mouseData;           /* This is where mouse service routines should look */
...
extern volatile uint16_t random;    /* random number, incremented each interrupt */
extern uint8_t* saveFontTab;        /* when going into menus, save user active font table here */
...
extern uint8_t* dlgBoxRamBuf;       /* buffer to hold variables while DB or DA is running
                                     * (to 0x8697) */
...
extern uint8_t driveData[4];        /* 1 byte each reserved for disk drives
                                     * about each device (each driver may use differently) */
extern uint8_t ramExpSize;
extern uint8_t sysRAMFlg;
extern uint8_t firstBoot;           /* This flag is changed from 0 to 0xff after deskTop comes 
                                     * up for the first time after booting */
...
extern uint8_t* objPointer[8];      /* Addresses of pointers to sprite object graphics */
...
extern uint8_t sysFlgCopy;          /* copy of sysRAMFlg saved here when going to BASIC */
