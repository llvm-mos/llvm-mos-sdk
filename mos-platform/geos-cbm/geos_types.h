...
typedef struct file_header
{
    ...
    uint16_t init_prog;              /* Address pf initialization routine to call after loading the program */
    char file_name[19];              /* 19 bytes ASCII application filename. Byte 0-11 = the name
                                      * padded with spaces; 12-15=version string "V1.3"; 16-20 = 0's */
    uint8_t c128_flags;              /* 1 byte, flags to indicate if this program will run under the C128 OS
                                      * in 40 column and in 80 column. These flags are valid for applications,
                                      * desk accessories, and auto-exec files. Bit 7: zero if runs in 40 column.
                                      * Bit 6: one if runs in 80 column. */
    char par_disk_or_author[20];     /* If data file, 20 bytes ASCII filename of parent's application disk.
                                      * if application program, holds name of software desginer. */
    char par_application[20];        /* If data file, 20 bytes parent application filename. Byte 0-11=name padded
                                      * with spaces; 12-15=version string "V1.3"; 16-20 = 0's */
    char application_use[23];        /* 23 bytes for application use */
    char notes[96];                  /* Used for file menu option getInfo. String must be null terminated and must
                                      * be first char if string is empty */
} file_header_t;

_Static_assert(sizeof(file_header_t) == 254, "Illegal size of net_file_header_t");

typedef struct file_header_block
{                             /* GEOS file header structure */
    tr_se_pair_t block_chain; /* Alyways 0/0xff: indicates last block, last used bytes at 0xff */
    file_header_t h;
} file_header_block_t;
