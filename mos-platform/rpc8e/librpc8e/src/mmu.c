...
void rpc8e_redbus_set_map(uint8_t device_id) {
  __attribute__((leaf)) asm volatile("mmu #$00" : "+a"(device_id) : :);
}

uint8_t rpc8e_redbus_get_map(void) {
  uint8_t result;
  __attribute__((leaf)) asm volatile("mmu #$80" : "=a"(result) : :);
  return result;
}
...
void rpc8e_redbus_window_set_offset(uint16_t offset) {
  ...
  __attribute__((leaf)) asm volatile("xba\n"
                                     "txa\n"
                                     "mmu #$01"
                                     : "+a"(offset_hi), "+x"(offset_lo)
                                     :
                                     : "p");
}
...
uint16_t rpc8e_redbus_window_get_offset(void) {
  ...
  __attribute__((leaf)) asm volatile("mmu #$81\n"
                                     "txa\n"
                                     "xba"
                                     : "=a"(result_hi), "=x"(result_lo)
                                     :
                                     : "p");
...
uint16_t rpc8e_mmu_ext_window_get(void) {
  uint8_t result_hi, result_lo;
  __attribute__((leaf)) asm volatile("mmu #$83\n"
                                     "txa\n"
                                     "xba"
                                     : "=a"(result_hi), "=x"(result_lo)
                                     :
...
uint16_t rpc8e_mmu_get_brk_address(void) {
  uint8_t result_hi, result_lo;
  __attribute__((leaf)) asm volatile("mmu #$85\n"
                                     "txa\n"
                                     "xba"
                                     : "=a"(result_hi), "=x"(result_lo)
                                     :
...
uint16_t rpc8e_mmu_get_por_address(void) {
  uint8_t result_hi, result_lo;
  __attribute__((leaf)) asm volatile("mmu #$86\n"
                                     "txa\n"
                                     "xba"
                                     : "=a"(result_hi), "=x"(result_lo)
                                     :
