...
volatile const char large_array[0x1000] = {[0] = 1, [0x0fff] = 2};
...
int main(void) {
  if ((unsigned)large_array < 0xe000)
    return EXIT_FAILURE;
  if (large_array[0] != 1)
    return EXIT_FAILURE;
  if (large_array[0x0fff] != 2)
    return EXIT_FAILURE;

  if ((unsigned)bank_0 < 0xc000)
    return EXIT_FAILURE;
  set_prg_8000(0);
  if (bank_0[0] != 1)
    return EXIT_FAILURE;
